...
- 결과 GUI 창 출력 (`--show`)
- 한글·유니코드 경로 지원 (`np.fromfile` + `cv2.imdecode` 방식)
...
7장의 이미지가 하나의 넓은 파노라마로 합성된 결과입니다.
...
7장의 이미지를 중앙(#3)을 기준으로 정렬합니다.

```
이미지 순서:  0 → 1 → 2 → 3(기준) ← 4 ← 5 ← 6

H_abs[3] = I                              (기준, 항등 행렬)
H_abs[2] = H_23
H_abs[1] = H_23 × H_12
H_abs[0] = H_23 × H_12 × H_01
H_abs[4] = H_34⁻¹
H_abs[5] = H_34⁻¹ × H_45⁻¹
H_abs[6] = H_34⁻¹ × H_45⁻¹ × H_56⁻¹
```
